...
const componentLib = require.context("./components", true, /\.js$/);
// use the require context to provide a mapping of component names to imports
const Spawn = importModules(componentLib, (key, item) => {
    return {
...
function reMount (group) {
    ...
    return register.get(group).reduceRight((status, target, ndx) => {
...
function create ({parent, props, type, name, node, config = {}}) {
    ...
    let target = this.components.find(c => c.type === type);
    // throw an error if <type> is not in the JSON config
    if (target === undefined) {
        throw new Error(`component type "${type}" is missing a config`);
    }
    // throw an error if <name> is already taken
    ...
    if (target.config) {
        ...
        try {
            // throw an error if target.instanceOf is missing
            target = target.instanceOf;
        } catch (err) {
            throw err;
        }
    }
    // "inherited" props delegates lookups to a parent node in the app tree
    if (props) {
        // re-defining props will break the chain of prototypal inheritence
        config = Object.assign(Object.create(props), config);
    }
    // tag the component's node
    node.setAttribute("id", name);
    // create a new component from the provided options
    instances.set(name, new Spawn[target](register, create.bind(this), reMount, defaults, config));
...
export function init (versionName, parentNode, options) {
    // find the app defaults
    const defaults = appConfig.versions.find(v => v.name === versionName);
    // fail fast
    if (!defaults) {
        throw new Error("version name not found in appconfig");
    }
