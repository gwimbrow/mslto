export default class Core {
    ...
    constructor (register, create, reMount, defaults, config = {}) {
        ...
        Object.defineProperties(_this, Object.assign({}, defaults, {
            ...
            // context makes use of a proxy for config to handle operations for
            // the application's "state"
            "context": { value: new Proxy(config, {
                ...
                set (target, key, val) {
                    ...
                    // if the parent is defined, delegate to that instance
                        ...
                        return !!(_this.parent.context[key] = val);
                    }
                },
                ...
                deleteProperty (target, key) {
                    ...
                    if (target.hasOwnProperty(key)) {
                        ...
                        const group = `${progenitor.name}.${key}`;
                        if (progenitor !== _this) {
                            // this will happen when a node is masking a prop
                            // in the prototype chain. we need to append the
                            // nodes registered for the deprecated group onto
                            // any pre-existing nodes associated with the masked
                            // prop. unlikely in practice, since the prop being
                            // masked must have been defined after instantiation
                            // time, or as a duplicate in the component config
                            const deprecated = `${_this.name}.${key}`;
        ...
        // trigger the "created" hook, which saves us from needing to call
        // super() with arguments from a subclassed constructor function
        this.manifest();
    }

    findProgenitor (key) {
        if (this.parent && this.parent.context[key]) {
    ...
    manifest () {
        // not to be confused with .create()
        // should be used to handle operations at instantiation time
    }
    ...
    mount () {
        ...
        // mount () {
        //     return this.parse `template literal`;
        // }
